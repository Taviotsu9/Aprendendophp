...
if (isset($_GET["id"]) && is_numeric($_GET["id"])) //Incluiaqui
{
    //pega aqui o id passado
    $id = (int) $_GET["id"];
    ...
    //passa o id como parâmetro, do tipo i = int, inteiro
    $stmt->bind_param("i", $id);

    //direciona para executar a consulta...
    $stmt->execute();

    //...retorna ao resultado, e atribuí à variável $result
    $result = $stmt->get_result();

    
    //por meio do método fetch_assoc, ele realiza uma associação de valores
    $aux_query = $result->fetch_assoc();

    //aqui atribuimos variaveis
    $nome = $aux_query["Nome"];
